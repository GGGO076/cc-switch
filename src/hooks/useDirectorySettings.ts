...
> = {
  ...
  prime: { key: "prime", defaultFolder: ".prime/agent" },
};
    ...
    const load = async () => {
      try {
        const [
          ...
          settingsApi.getConfigDir("prime"),
        ...
        setResolvedDirs({
          ...
          prime: primeDir || defaultsRef.current.prime,
        });
      } catch (error) {
        console.error(
  ...
  const resetAllDirectories = useCallback(
    (overrides?: ResolvedAppDirectoryOverrides) => {
      ...
      setResolvedDirs({
        ...
        prime: overrides?.prime ?? defaultsRef.current.prime,
      });
    },
    [],
  );
