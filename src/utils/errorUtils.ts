...
export const translateOmpProviderMutationError = (
  message: string,
  t: (key: string, options?: Record<string, unknown>) => string,
): string => {
  if (!message) return "";
  ...
  if (
    ...
    return t("omp.provider.writeConflict");
  }

  if (
    message.includes("already exists") &&
    /omp provider/i.test(message)
  ) {
    return t("omp.form.providerKeyDuplicate");
  }

  return "";
};
export const translatePrimeProviderMutationError = (
  message: string,
  t: (key: string, options?: Record<string, unknown>) => string,
): string => {
  if (!message) return "";
  ...
  if (
    ...
    return t("prime.provider.writeConflict");
  }

  if (
    message.includes("already exists") &&
    /prime provider/i.test(message)
  ) {
    return t("prime.form.providerKeyDuplicate");
  }
