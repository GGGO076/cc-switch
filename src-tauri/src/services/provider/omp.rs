...
pub(super) fn add(
    ...
    if !add_to_live && crate::omp_config::omp_provider_exists(&provider.id)? {
        return Err(AppError::InvalidInput(format!(
            "OMP provider key '{}' already exists in models.json",
            provider.id
        )));
    }
...
mod tests {
    ...
    fn membership_is_derived_only_from_models_json() {
        ...
        ProviderService::add(&state, AppType::Omp, input("model-a"), false)
    ...
    #[test]
    #[serial]
    fn default_role_points_at_enabled_provider_but_never_blocks_membership() {
        let _agent = TestAgentDir::new();
        let state = state();
        let original = input("model-a");
        ProviderService::add(&state, AppType::Omp, original.clone(), true).expect("add provider");
        assert_eq!(
            crate::omp_config::read_omp_default_role().expect("read default role"),
            None,
            "adding to live config alone selects no default role"
        );
        let config_path = crate::omp_config::get_omp_config_path().unwrap();
        fs::write(
            &config_path,
            "modelRoles:\n  default: cc-switch-test/model-a\n",
        )
        .unwrap();
        ...
        assert_eq!(
            ...
            "edits leave the default role alone"
        );
        ProviderService::remove_from_live_config(&state, AppType::Omp, "cc-switch-test")
        ...
        assert_eq!(
            ...
            "enabling points the default role at the first model"
        );
        ...
        assert_eq!(
            crate::omp_config::read_omp_default_role().expect("read default role"),
            Some("cc-switch-test/model-b".to_string()),
            "deletion leaves the advisory default role alone"
        );
        assert!(!crate::omp_config::omp_provider_exists("cc-switch-test").unwrap());
    }
    ...
    #[test]
    #[serial]
    fn provider_membership_never_changes_omp_vault_or_unrelated_config() {
        let _agent = TestAgentDir::new();
        let state = state();
        ...
        assert_eq!(
            ...
            "removal leaves the default role alone"
        );
        ...
        assert_eq!(
            ...
            config_contents,
            "a modelless provider never rewrites unrelated config"
        );
        assert!(
            !agent_dir.join("agent.db").exists(),
            "membership changes must never create the sqlite vault"
        );
    }
    ...
    fn failed_duplicate_create_rolls_back_native_insertion() {
        ...
        assert!(ProviderService::add(&state, AppType::Omp, input("model-a"), true).is_err());
        assert!(!crate::omp_config::omp_provider_exists("cc-switch-test").unwrap());
    }
    ...
    fn native_edits_sync_to_the_saved_provider_and_survive_removal() {
        let _agent = TestAgentDir::new();
        let state = state();
        ProviderService::add(&state, AppType::Omp, input("model-a"), true).expect("add provider");
        let saved = state
            .db
            .get_provider_by_id("cc-switch-test", "omp")
            .unwrap()
    ...
    fn refreshed_native_config_can_be_edited_without_snapshot_state() {
        ...
        ProviderService::add(&state, AppType::Omp, baseline.clone(), true).expect("add provider");
    ...
    fn enabled_provider_edit_needs_no_special_snapshot_parameter() {
        ...
        ProviderService::update(&state, AppType::Omp, Some("cc-switch-test"), edited.clone())
    ...
    fn copied_provider_keeps_its_display_name_after_enable_and_sync() {
        ...
        ProviderService::add(&state, AppType::Omp, copy, false).expect("save copied provider");
    ...
    fn database_only_create_does_not_overwrite_an_unsynced_native_key() {
        ...
        let mut copy = input("model-a");
        copy.id = "cc-switch-test-copy".to_string();
        let error = ProviderService::add(&state, AppType::Omp, copy, false)
    ...
    fn malformed_native_file_keeps_the_saved_catalog_visible() {
        let _agent = TestAgentDir::new();
        let state = state();
        ProviderService::add(&state, AppType::Omp, input("model-a"), false).expect("save provider");
        let path = crate::omp_config::get_omp_models_path().expect("models path");
        fs::create_dir_all(path.parent().expect("models directory"))
            .expect("create models directory");
    ...
    fn numeric_json_representation_does_not_block_removal() {
        ...
        ProviderService::add(&state, AppType::Omp, saved, false).expect("save provider");
    ...
    #[test]
    #[serial]
    fn unreadable_default_config_blocks_only_enable_and_rolls_back() {
        let _agent = TestAgentDir::new();
        let state = state();
        let config_path = crate::omp_config::get_omp_config_path().unwrap();
        fs::create_dir_all(config_path.parent().unwrap()).unwrap();
        fs::write(&config_path, "{not-yaml: [unclosed").unwrap();

        ProviderService::add(&state, AppType::Omp, input("model-a"), true)
        ...
        update(&state, Some("cc-switch-test"), original.clone())
            .expect("an edit that keeps every model does not need the default role");

        ProviderService::remove_from_live_config(&state, AppType::Omp, "cc-switch-test")
            .expect("global selection is advisory for removal");
        let error = ProviderService::switch(&state, AppType::Omp, "cc-switch-test")
            .expect_err("enabling without a writable default role must fail");
        assert!(
            error.to_string().contains("OMP config"),
    ...
    #[test]
    #[serial]
    fn enable_points_default_role_at_first_model_and_keeps_thinking_tail() {
        let _agent = TestAgentDir::new();
        let state = state();
        ProviderService::add(&state, AppType::Omp, input("model-a"), false)
            .expect("save disabled provider");
        ...
        assert_eq!(
            crate::omp_config::read_omp_default_role().expect("read default role"),
            Some("cc-switch-test/model-a:xhigh".to_string())
        );
        let text = fs::read_to_string(&config_path).expect("read config");
        assert!(text.contains("other: keep"), "other keys survive: {text}");
    }
}
