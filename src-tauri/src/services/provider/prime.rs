...
pub(super) fn list(state: &AppState) -> Result<IndexMap<String, Provider>, AppError> {
    ...
    match crate::prime_config::read_prime_native_providers() {
        Ok(native) => {
            if let Err(error) = sync_native_locked(state, &native) {
                log::warn!("Failed to sync Prime providers from native config: {error}");
            }
        }
        Err(error) => {
            log::warn!("Failed to read Prime providers; showing saved catalog: {error}");
        }
    }
    state.db.get_all_providers(PRIME_APP)
}

pub(super) fn import_from_live(state: &AppState) -> Result<usize, AppError> {
    let _guard = futures::executor::block_on(state.proxy_service.lock_switch_for_app(PRIME_APP));
    let native = crate::prime_config::read_prime_native_providers()?;
...
pub(super) fn add(
        ...
        .is_some()
    {
        return Err(AppError::InvalidInput(format!(
            "Prime provider '{}' already exists",
            provider.id
        )));
    }

    if !add_to_live && crate::prime_config::prime_provider_exists(&provider.id)? {
        return Err(AppError::InvalidInput(format!(
            "Prime provider key '{}' already exists in models.json",
            provider.id
        )));
    }

    let native_inserted = if add_to_live {
        crate::prime_config::insert_prime_provider(&provider.id, &provider.settings_config)?
    } else {
        false
    };
    ...
    if let Err(error) = state.db.save_provider(app_type.as_str(), &provider) {
        if native_inserted {
            if let Err(rollback) = crate::prime_config::remove_prime_provider_if_matches(
                &provider.id,
                &provider.settings_config,
            ) {
                return Err(AppError::Config(format!(
                    "failed to save Prime provider: {error}; native rollback failed: {rollback}"
                )));
            }
        }
        return Err(error);
...
pub(super) fn update_usage_script(
    ...
    let mut provider = state
        ...
        .ok_or_else(|| AppError::InvalidInput(format!("Prime provider '{id}' not found")))?;
    provider
        .meta
...
pub(super) fn update(
    ...
    if original_id != provider.id {
        return Err(AppError::InvalidInput(
            "Prime provider keys cannot be renamed".to_string(),
        ));
    }
    ...
    state
        ...
        .ok_or_else(|| AppError::InvalidInput(format!("Prime provider '{original_id}' not found")))?;
    strip_unsupported_prime_metadata(&mut provider);
    ProviderService::validate_provider_settings(&app_type, &provider)?;
    ProviderService::normalize_usage_script_credential_overrides(&app_type, &mut provider);

    let previous_native =
        crate::prime_config::replace_prime_provider_if_present(&original_id, &provider.settings_config)?;
    if let Err(error) = state.db.save_provider(app_type.as_str(), &provider) {
        if let Some(previous_native) = previous_native.as_ref() {
            if let Err(rollback) = crate::prime_config::replace_prime_provider(
                &original_id,
                &provider.settings_config,
                previous_native,
            ) {
...
pub(super) fn delete(state: &AppState, id: &str) -> Result<(), AppError> {
    ...
    let removed = crate::prime_config::remove_prime_provider(id)?;

    if let Err(error) = state.db.delete_provider(app_type.as_str(), id) {
        if let Some(removed) = removed.as_ref() {
            if let Err(rollback) = crate::prime_config::restore_prime_provider_if_missing(id, removed) {
                return Err(AppError::Config(format!(
                    "failed to delete Prime provider: {error}; native rollback failed: {rollback}"
                )));
            }
        }
        return Err(error);
...
pub(super) fn remove(state: &AppState, id: &str) -> Result<(), AppError> {
    ...
    let provider = state
        ...
        .get_provider_by_id(id, app_type.as_str())?
        .ok_or_else(|| AppError::InvalidInput(format!("Prime provider '{id}' not found")))?;
    let Some(removed) = crate::prime_config::remove_prime_provider(id)? else {
        return Ok(());
    };
    ...
    if let Err(error) = state.db.save_provider(app_type.as_str(), &synced) {
        if let Err(rollback) = crate::prime_config::restore_prime_provider_if_missing(id, &removed) {
            return Err(AppError::Config(format!(
                "failed to preserve Prime provider before removal: {error}; native rollback failed: {rollback}"
            )));
        }
        return Err(error);
    }
...
pub(super) fn enable(state: &AppState, id: &str) -> Result<SwitchResult, AppError> {
    ...
    if let Some(native) = crate::prime_config::read_prime_native_provider(id)? {
        let mut synced = provider;
    ...
    ProviderService::validate_provider_settings(&app_type, &provider)?;
    crate::prime_config::insert_prime_provider(id, &provider.settings_config)?;
...
fn sync_native_locked(
    ...
    let saved = state.db.get_all_providers(PRIME_APP)?;
    let mut changed = 0;
    ...
    for (id, config) in native {
        ...
        state.db.save_provider(PRIME_APP, &provider)?;
        changed += 1;
    }
...
fn strip_unsupported_prime_metadata(provider: &mut Provider) {
...
mod tests {
    ...
    fn membership_is_derived_only_from_models_json() {
        ...
        ProviderService::add(&state, AppType::Prime, input("model-a"), false)
            .expect("save disabled provider");
        assert!(!crate::prime_config::prime_provider_exists("cc-switch-test").unwrap());

        let saved = state
            .db
            .get_provider_by_id("cc-switch-test", "prime")
            .unwrap()
            .unwrap();
        ...
        ProviderService::switch(&state, AppType::Prime, "cc-switch-test").expect("enable provider");
        assert!(crate::prime_config::prime_provider_exists("cc-switch-test").unwrap());

        ProviderService::remove_from_live_config(&state, AppType::Prime, "cc-switch-test")
    ...
    fn default_selection_does_not_block_membership_changes() {
        ...
        ProviderService::add(&state, AppType::Prime, original.clone(), true).expect("add provider");
        let settings_path = crate::prime_config::get_prime_settings_path().unwrap();
        ...
        ProviderService::switch(&state, AppType::Prime, "cc-switch-test").expect("re-enable provider");
        ProviderService::delete(&state, AppType::Prime, "cc-switch-test")
    ...
    fn provider_membership_never_changes_pi_auth_or_defaults() {
        let _agent = TestAgentDir::new();
        let state = state();
        let agent_dir = crate::prime_config::get_prime_agent_dir().expect("agent directory");
        ...
        ProviderService::list(&state, AppType::Prime).expect("import explicit provider");
        ProviderService::remove_from_live_config(&state, AppType::Prime, "anthropic")
            .expect("remove explicit provider");
        ProviderService::switch(&state, AppType::Prime, "anthropic")
            .expect("enable explicit provider");
        let mut edited = state
            .db
            .get_provider_by_id("anthropic", PRIME_APP)
    ...
    fn failed_duplicate_create_rolls_back_native_insertion() {
        ...
        assert!(ProviderService::add(&state, AppType::Prime, input("model-a"), true).is_err());
        assert!(!crate::prime_config::prime_provider_exists("cc-switch-test").unwrap());
    }
    ...
    fn native_edits_sync_to_the_saved_provider_and_survive_removal() {
        let _agent = TestAgentDir::new();
        let state = state();
        ProviderService::add(&state, AppType::Prime, input("model-a"), true).expect("add provider");
        let saved = state
            .db
            .get_provider_by_id("cc-switch-test", "prime")
            .unwrap()
        ...
        crate::prime_config::replace_prime_provider("cc-switch-test", &saved.settings_config, &external)
            .expect("edit native provider");

        let listed = ProviderService::list(&state, AppType::Prime).expect("sync native providers");
    ...
    fn refreshed_native_config_can_be_edited_without_snapshot_state() {
        ...
        ProviderService::add(&state, AppType::Prime, baseline.clone(), true).expect("add provider");
        ...
        crate::prime_config::replace_prime_provider(
            "cc-switch-test",
        ...
        let listed = ProviderService::list(&state, AppType::Prime).expect("refresh native provider");
        ...
        assert_eq!(
            crate::prime_config::read_prime_native_provider("cc-switch-test")
    ...
    fn enabled_provider_edit_needs_no_special_snapshot_parameter() {
        ...
        ProviderService::update(&state, AppType::Prime, Some("cc-switch-test"), edited.clone())
    ...
    fn native_sync_imports_every_explicit_provider_node() {
        ...
        ProviderService::add(&state, AppType::Prime, stale_oauth, false).expect("save stale provider");
        let path = crate::prime_config::get_prime_models_path().unwrap();
        ...
        let providers = ProviderService::list(&state, AppType::Prime).expect("sync providers");
    ...
    fn removal_preserves_and_can_restore_a_minimal_native_node() {
        ...
        let path = crate::prime_config::get_prime_models_path().expect("models path");
        fs::write(
            &path,
        ...
        assert_eq!(
            state
                .db
                .get_provider_by_id("cc-switch-test", PRIME_APP)
        ...
        ProviderService::switch(&state, AppType::Prime, "cc-switch-test")
    ...
    fn usage_metadata_update_does_not_rewrite_native_provider_settings() {
        ...
        let providers = ProviderService::list(&state, AppType::Prime).expect("sync provider");
    ...
    fn copied_provider_keeps_its_display_name_after_enable_and_sync() {
        ...
        ProviderService::add(&state, AppType::Prime, copy, false).expect("save copied provider");
        ProviderService::switch(&state, AppType::Prime, "cc-switch-test-copy")
    ...
    fn database_only_create_does_not_overwrite_an_unsynced_native_key() {
        ...
        let mut copy = input("model-a");
        copy.id = "cc-switch-test-copy".to_string();
        let error = ProviderService::add(&state, AppType::Prime, copy, false)
        ...
        assert!(state
            .db
            .get_provider_by_id("cc-switch-test-copy", PRIME_APP)
            .expect("read saved provider")
            .is_none());
        assert!(crate::prime_config::prime_provider_exists("cc-switch-test-copy")
            .expect("read native provider"));

        let providers = ProviderService::list(&state, AppType::Prime).expect("sync native provider");
    ...
    fn malformed_native_file_keeps_the_saved_catalog_visible() {
        let _agent = TestAgentDir::new();
        let state = state();
        ProviderService::add(&state, AppType::Prime, input("model-a"), false).expect("save provider");
        let path = crate::prime_config::get_prime_models_path().expect("models path");
        fs::create_dir_all(path.parent().expect("models directory"))
            .expect("create models directory");
        fs::write(path, "{not-json").expect("write malformed models");

        let providers = ProviderService::list(&state, AppType::Prime).expect("read saved catalog");
    ...
    fn numeric_json_representation_does_not_block_removal() {
        ...
        ProviderService::add(&state, AppType::Prime, saved, false).expect("save provider");
    ...
    fn unreadable_selection_does_not_block_membership_changes() {
        ...
        ProviderService::add(&state, AppType::Prime, input("model-a"), true)
