//! Thin adapter for Prime's native files.
//!
//! Prime owns account login and the active provider/model in `settings.json`.
...
const MAX_PRIME_FILE_BYTES: u64 = 1024 * 1024;
...
pub(crate) struct PrimeNativeDefaults {
...
pub(crate) fn get_prime_agent_dir() -> Result<PathBuf, AppError> {
    #[cfg(test)]
...
pub(crate) fn get_prime_models_path() -> Result<PathBuf, AppError> {
    Ok(get_prime_agent_dir()?.join("models.json"))
}

pub(crate) fn get_prime_settings_path() -> Result<PathBuf, AppError> {
    Ok(get_prime_agent_dir()?.join("settings.json"))
}

pub(crate) fn read_prime_native_defaults() -> Result<PrimeNativeDefaults, AppError> {
    let path = get_prime_settings_path()?;
    if !path.exists() {
        return Ok(PrimeNativeDefaults::default());
    }
    let value = read_json5_value(&path, "Prime settings")?;
    let object = value.as_object().ok_or_else(|| {
        AppError::Config(format!(
            "Prime settings root must be an object: {}",
            path.display()
        ))
    })?;
    Ok(PrimeNativeDefaults {
...
pub(crate) fn read_prime_native_providers() -> Result<IndexMap<String, Value>, AppError> {
    let _guard = lock_models_file()?;
    read_prime_native_providers_locked(&get_prime_models_path()?)
}

pub(crate) fn read_prime_native_provider(provider_key: &str) -> Result<Option<Value>, AppError> {
    let _guard = lock_models_file()?;
    let path = get_prime_models_path()?;
    let document = read_models_document(&path)?;
    Ok(providers(&document, &path)?.get(provider_key).cloned())
}

pub(crate) fn prime_provider_exists(provider_key: &str) -> Result<bool, AppError> {
    let _guard = lock_models_file()?;
    let path = get_prime_models_path()?;
    let document = read_models_document(&path)?;
...
pub(crate) fn insert_prime_provider(provider_key: &str, config: &Value) -> Result<bool, AppError> {
    ...
    match providers.get(provider_key) {
        ...
        Some(_) => {
            return Err(AppError::InvalidInput(format!(
                "Prime provider key '{provider_key}' already exists in models.json"
            )))
        }
        None => {}
    }
...
pub(crate) fn replace_prime_provider(
    provider_key: &str,
    expected: &Value,
    replacement: &Value,
) -> Result<(), AppError> {
    ...
    let current = providers.get(provider_key).ok_or_else(|| {
        AppError::Conflict(format!(
            "Prime provider '{provider_key}' is no longer present in models.json"
        ))
    })?;
    if current != expected {
        return Err(AppError::Conflict(format!(
            "Prime provider '{provider_key}' changed outside CC Switch"
        )));
    }
...
pub(crate) fn replace_prime_provider_if_present(
    provider_key: &str,
    replacement: &Value,
...
pub(crate) fn remove_prime_provider(provider_key: &str) -> Result<Option<Value>, AppError> {
    remove_prime_provider_inner(provider_key, None)
}

pub(crate) fn remove_prime_provider_if_matches(
    provider_key: &str,
    expected: &Value,
) -> Result<bool, AppError> {
    remove_prime_provider_inner(provider_key, Some(expected)).map(|removed| removed.is_some())
}

fn remove_prime_provider_inner(
    provider_key: &str,
...
pub(crate) fn restore_prime_provider_if_missing(
    provider_key: &str,
    config: &Value,
) -> Result<(), AppError> {
    let _guard = lock_models_file()?;
    ...
    match providers.get(provider_key) {
        ...
        Some(_) => Err(AppError::Conflict(format!(
            "cannot restore Prime provider '{provider_key}' because another value now owns the key"
        ))),
        None => {
            providers.insert(provider_key.to_string(), config.clone());
            write_models_document(&path, &document, &expected_revision)
...
/// `models.json.providers` is manageable, including keys also built into Prime.
/// Prime's `/login` credentials live in `auth.json` and are never read here.
pub(crate) fn validate_provider_node(provider_key: &str, config: &Value) -> Result<(), AppError> {
    if provider_key.trim().is_empty() {
        return Err(AppError::InvalidInput(
            "Prime provider key cannot be empty".to_string(),
        ));
    }
    config.as_object().ok_or_else(|| {
        AppError::InvalidInput("Prime provider configuration must be an object".to_string())
    })?;
    Ok(())
}
...
pub(crate) fn provider_base_url(config: &Value) -> Result<String, AppError> {
    ...
    nonempty_string(provider.get("baseUrl"))
        ...
        .map(str::to_string)
        .ok_or_else(|| AppError::InvalidInput("Prime provider has no request URL".to_string()))
}

fn lock_models_file() -> Result<MutexGuard<'static, ()>, AppError> {
    MODELS_FILE_LOCK
        .lock()
        .map_err(|error| AppError::Config(format!("Prime models file lock is poisoned: {error}")))
}

fn read_prime_native_providers_locked(path: &Path) -> Result<IndexMap<String, Value>, AppError> {
    let document = read_models_document(path)?;
...
fn read_models_document_with_revision(path: &Path) -> Result<(Value, String), AppError> {
    ...
    let bytes = read_file_limited(path, "Prime models")?;
    let revision = revision(&bytes);
    let document = parse_json5_value(path, "Prime models", bytes)?;
...
fn read_file_limited(path: &Path, label: &str) -> Result<Vec<u8>, AppError> {
    ...
    if metadata.len() > MAX_PRIME_FILE_BYTES {
        return Err(AppError::InvalidInput(format!(
    ...
    file.take(MAX_PRIME_FILE_BYTES + 1)
        .read_to_end(&mut bytes)
        .map_err(|error| AppError::io(path, error))?;
    if bytes.len() as u64 > MAX_PRIME_FILE_BYTES {
        return Err(AppError::InvalidInput(format!(
            "{label} file exceeds the 1 MiB limit: {}",
            path.display()
        )));
...
fn providers<'a>(document: &'a Value, path: &Path) -> Result<&'a Map<String, Value>, AppError> {
    let root = document.as_object().ok_or_else(|| {
        AppError::Config(format!(
            "Prime models root must be an object: {}",
            path.display()
        ))
    })?;
    match root.get("providers") {
        ...
        Some(_) => Err(AppError::Config(format!(
            "Prime models 'providers' must be an object: {}",
            path.display()
        ))),
    }
}
...
fn ensure_models_revision(path: &Path, expected_revision: &str) -> Result<(), AppError> {
    let actual_revision = match fs::File::open(path) {
        Ok(_) => revision(&read_file_limited(path, "Prime models")?),
    ...
    if actual_revision == expected_revision {
        ...
        Err(AppError::Conflict(format!(
            "Prime models.json changed outside CC Switch: {}",
            path.display()
        )))
    }
}
...
fn ensure_private_models_parent(path: &Path) -> Result<(), AppError> {
    let parent = path.parent().ok_or_else(|| {
        AppError::Config(format!(
            "Prime models path has no parent directory: {}",
            path.display()
        ))
    })?;
...
fn optional_string(
    ...
    match object.get(key) {
        ...
        Some(_) => Err(AppError::Config(format!(
            "Prime settings '{key}' must be a string: {}",
            path.display()
        ))),
    }
}
    ...
    impl Drop for TestAgentDir {
        fn drop(&mut self) {
            *super::TEST_AGENT_DIR
                .lock()
                .expect("lock Prime test directory") = self.previous.take();
        }
    }
}
...
mod tests {
    ...
    fn relative_agent_directory_is_rejected() {
        let error = resolve_prime_agent_dir(
            None,
            Some("relative/prime-agent".into()),
            PathBuf::from("default"),
        )
        .expect_err("relative Prime directory must be rejected");
    ...
    fn settings_directory_precedes_the_environment() {
        ...
        assert_eq!(
            ...
            .expect("resolve Prime directory"),
            settings_dir
        );
    }
    ...
    fn duplicate_provider_key_is_validation_not_a_write_conflict() {
        ...
        insert_prime_provider("duplicate", &provider()).expect("insert provider");
        let mut replacement = provider();
        replacement["name"] = json!("Other");

        let error = insert_prime_provider("duplicate", &replacement)
    ...
    fn newly_created_models_file_and_agent_directory_are_private() {
        ...
        let _agent = test_support::TestAgentDir::new();
        insert_prime_provider("cc-switch-private", &provider()).expect("write private models file");

        let path = get_prime_models_path().expect("models path");
    ...
    fn stale_models_revision_does_not_overwrite_an_external_edit() {
        ...
        let external = r#"{"providers":{"external":{"models":[]},"prime-added":{"models":[]}}}"#;
